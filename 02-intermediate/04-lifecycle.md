...
**🎯 학습 목표**
- React의 `useEffect` vs Vue의 라이프사이클 훅 비교 및 이해
- 각 라이프사이클 단계의 적절한 활용법 습득 (데이터 로딩, 정리 등)
- Composition API에서의 라이프사이클 훅 사용법 완전 정복
- 실무에서 자주 사용하는 라이프사이클 패턴들 익히기

**💡 왜 Vue의 라이프사이클이 더 명확할까요?**
- **명시적 단계**: 각 단계가 명확하게 구분되어 언제 어떤 작업을 해야 할지 명확
- **직관적 이름**: `onMounted`, `onUpdated` 등 이름만 봐도 언제 실행되는지 바로 이해
- **컴포넌트별 관리**: Options API에서 라이프사이클 옵션이 명확히 분리
- **디버깅 용이**: Vue DevTools에서 라이프사이클 실행 과정 추적 가능
